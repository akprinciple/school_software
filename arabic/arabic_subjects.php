    ...
    if (isset($_GET['success'])) {
      $msg = "<script>alert('Update Successful');</script>";
		...
		if (isset($_POST['submit'])) {
			...
			$sel = "SELECT * FROM arabic_subjects WHERE subject = '{$subject}'";
			$selt = mysqli_query($connect, $sel);
			$count = mysqli_num_rows($selt);
			if ($count > 0) {
				$msg = "<div class='p-2 rounded text-danger text-center mb-2'>Subject is already existing</div>";
			}
			else{
			$sql = "INSERT INTO arabic_subjects (subject) VALUES ('$subject')";
			$query = mysqli_query($connect, $sql);
			if ($query) {
				$msg = "<div class='p-2 rounded text-success text-center mb-2'>subject Added Successfully</div>";
			}
			else{
...
<html>
<head>
	<title>Arabic Subjects | Queens School</title>
...
<h4 class="ml-3 mt-3 text-center">﷽</h4>
             ...
             <div class="">
           		<div class="col-md-9 m-auto">
           				...
           				<form method="post" enctype="multipart/form-data">
           					<h5 class="">Add Subjects(الموا د)</h5>
           					<?php echo $msg; ?>
           					<div class="form-group mt-2 row p-3">
           						<input type="text" name="subject" class="form-control w-75" placeholder="Add Subject" required="required">
           			...
           			<div class="">
           				...
           				<table class="table table-striped text-center col-md-12 border">
           					<thead class="bg-success text-light">
           						<tr>
           							<th>S/N</th>
           							<th>Subjects(الموا د)</th>
           							<th>Action</th>
           						</tr>
           					</thead>
           					<tbody>
           						<?php 
           							$sel = "SELECT * FROM arabic_subjects";
									$selt = mysqli_query($connect, $sel);
									$n = 1;
        ...
        <?php 
 $u_sql = "SELECT * FROM arabic_subjects";
...
<?php 
  if (isset($_POST['edit'.$id])) {
    ...
    $subj = $_POST['subj'.$id];
    $update = mysqli_query($connect, "UPDATE arabic_subjects SET subject = '{$subj}' WHERE id = '{$id}'");
    if ($update) {
      header('location: arabic_subjects?success');
      // echo "Successful";
    }
  }
