	 ...
	 ?>
                    ...
                    <div class="col-md-6 mt-4">
                        ...
                        <button type="search" name="submit"
                            class="btn btn-success d-block rounded w-100">Search</button>
                    </div>
                </form>
                ...
                <div class="col-md-4 form-group p-2">
                    <form>
                        ...
                        <br>
                        <input type="search" disabled required="required" name="search" placeholder="Search By keyword"
...
if (isset($_GET['class']) && isset($_GET['term']) && isset($_GET['session'])) {
	...
	?>
            ...
            <a
                href="print_all_terminalresults?class=<?php echo $class; ?>&session=<?php echo $session; ?>&term=<?php echo $term; ?>"><button
                    class="btn btn-danger float-right">Print All</button></a>
		...
		?>
            ...
            <table class="table-bordered table-hover table col-md-12 text-center m-auto"
	...
	?>
		...
		?>
                        ...
                        <td>
                            <!-- view button -->
                            <span id="up<?php echo $report['student_id']; ?>" title="View results"
	...
	while ($rw = mysqli_fetch_array($query)) {
	 ...
	 " title="Share on whatsapp" target="_blank" class="fab fa-whatsapp btn btn-success text-light p-1 desktop">
				 ...
				 ?>
                            ...
                            <span class="fas fa-check btn btn-warning p-1" title="Click to Approve"
                                onclick="appr<?php echo $report['student_id']; ?>()"
                                id="txtHint<?php echo $report['student_id']; ?>"></span>
                            <?php }else{ ?>
                            <span class="fas fa-check btn btn-primary p-1" title="click to Unapprove"
                                onclick="appr<?php echo $report['student_id']; ?>()"
                                id="txtHint<?php echo $report['student_id']; ?>"></span>
                            <?php } ?>
                            ...
                            function appr<?php echo $report['student_id']; ?>() {
                                ...
                                xmlhttp.onreadystatechange = function() {
                                    if (this.readyState == 4 && this.status == 200) {
                                        document.getElementById("txtHint<?php echo $report['student_id']; ?>")
                                            .className = this.responseText.split('|')[0];
                                        document.getElementById("txtHint<?php echo $report['student_id']; ?>")
                                            .title = this.responseText.split('|')[1];
                                    }
                                };
                                xmlhttp.open("GET",
                                    "parent_result?user=<?php echo $report['student_id']; ?>&class=<?php echo $class; ?>&session=<?php echo $session; ?>&term=<?php echo $term; ?>",
                                    true);
                                xmlhttp.send();
                            }
                            </script>
	...
	?>
    <div id="update<?php echo $user['student_id']; ?>" class="w-100 position-absolute position-fixe bg-white"
        style="background-color: rgba(0,0,0,0.1); min-height: 100%; top: 0; z-index: 2; overflow: auto; display : none;">
        ...
        <div class="col-md-11 m-auto p-3" style=" border: 5px dashed green;">
            ...
            <p class="w-75 mx-auto my-3  row text-uppercase" style=""> Name:
                <span class="w-75 text-center font-weight-bold "
                    style="border-bottom: 1px solid green; font-size: 20px;"><?php 
                ...
                <div class="w-50">
                    <p class="mx-0 row">
                        ...
                        <span class=" text-center font-weight-bold "
                            style="border-bottom: 1px solid green; width: 87%; font-size: 18px;">
                            <?php 
		$c_sql = "SELECT * FROM class WHERE id = '{$class}'";
		$c_query = mysqli_query($connect, $c_sql);
		while ($clas = mysqli_fetch_array($c_query)) {
			...
			?>
                ...
                <div class="w-50">
                    ...
                    <p class="mx-0 row">
                        ...
                        <span class=" text-center font-weight-bold text-capitalize"
                            style="border-bottom: 1px solid green; width: 87%; font-size: 18px;"><?php 
		$c_sql = "SELECT * FROM register WHERE id = '{$user["student_id"]}'";
		$c_query = mysqli_query($connect, $c_sql);
		while ($name = mysqli_fetch_array($c_query)) {
			 echo $name['gender'];
		...
		?>
                    ...
                    <p class="mx-0 row">
                        ...
                        <span class=" text-center font-weight-bold "
                            style="border-bottom: 1px solid green; width: 82%; font-size: 18px;">
                            <?php 
		 			...
		 			if($s['signature'] != ""){
						# code...
						 ...
						 ?>
                                ...
                                <img alt="<?php echo substr($s['name'], 0, 3) ?>"
                                    src="../signatures/<?php echo $s['signature'] ?>" width="50px">
                ...
                <p class="mx-0 mb-5 float-right mt-n5">
                    ...
                    <span class=" text-center font-weight-bold d-block" style="width: 200px">
		 			...
		 			while ($s = mysqli_fetch_array($sig)) {
                        ...
                        <img alt="<?php echo substr($s['name'], 0, 3) ?>"
                            src="../signatures/<?php echo "School Stamp Sec.png" ?>" width="100%">
                        <?php	}  else{ ?>
                        <img alt="<?php echo substr($s['name'], 0, 3) ?>"
                            src="../signatures/<?php echo $s['signature'] ?>" width="100%">
...
$n = 1;
	...
	while ($ter = mysqli_fetch_array($c_query)) {
        ...
        <div class=" bg-dark w-100 text-light text-center position-fixed p-2" style="bottom: 0; z-index: 2 ">
            ...
            <button type="button" class="btn btn-success" onclick="print(this)">Print <span
