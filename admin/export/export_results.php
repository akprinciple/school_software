...
if (isset($_GET['class']) && isset($_GET['term']) && isset($_GET['session'])) {
 ...
 $query = mysqli_query($connect, "SELECT * FROM mid_term_results WHERE class ='{$class}' && session = '{$session}' && term = '{$term}' ORDER BY  (SELECT name FROM register WHERE id = student_id) ASC");
...
$delimiter = ",";
$filename = "All results ".$class_row['class']." ".$term_row['term']." term ".$session_row['session'].".csv";

// Create a file pointer
$f = fopen('php://memory', 'w');

// Set column headers
$fields = array('S/N', 'STUDENT ID','NAME', 'SUBJECT', 'TERM', 'CLASS', 'SESSION', 'WEEKLT TEST', 'MIDTERM TEST', 'EXAM SCORE', 'TEACHER');
...
fpassthru($f);
	...
	header('location: ../class_results?status=empty');
	}
}
exit;
