...
<html>
<head>
	<title>View Midterm Results | Fidelity Schools</title>
...
<div class="col-md-10 ">
<h4 class="ml-3 mt-3">View Midterm Results</h4>
<?php 
include 'inc/hr.php';
...
if (isset($_GET['class']) && isset($_GET['term']) && isset($_GET['session'])) {
	...
	$select = "SELECT DISTINCT student_id FROM mid_term_results WHERE class ='{$class}'  && session = '{$session}'  && term = '{$term}' && test + test2 > 0";
	$show = mysqli_query($connect, $select);
	$count = mysqli_num_rows($show);
?>

<a href="midterm_print_all?class=<?php echo $class; ?>&session=<?php echo $session; ?>&term=<?php echo $term; ?>"><button class="btn btn-danger float-right">Print All</button></a>
...
<!-- The Print Preview -->
 ...
 $u_sql = "SELECT DISTINCT student_id FROM mid_term_results WHERE class ='{$class}'   && session = '{$session}'  && term = '{$term}' && test + test2 > 0 ORDER BY id DESC";
...
$n = 1;
	...
	while ($ter = mysqli_fetch_array($c_query)) {
	 	echo $ter['subject'];
		 	...
		 	<td><?php echo $rw['test2'] + $rw['test']; ?></td>
		 	<td>
			 <?php
				$highest_score = mysqli_query($connect, "SELECT MAX(test + test2) AS score FROM mid_term_results WHERE class ='{$class}'  && session = '{$session}'  && term = '{$term}' && subject = '{$rw['subject']}' && test + test2 > 0");
		 	...
		 	<td>
			 <?php
				$lowest_score = mysqli_query($connect, "SELECT MIN(test + test2) AS score FROM mid_term_results WHERE class ='{$class}'  && session = '{$session}'  && term = '{$term}' && subject = '{$rw['subject']}' && test + test2 > 0");
	 ...
	 <b class="" style="width: 17%"> Comment :</b>
			<span class=" text-center font-weight-bold " style="border-bottom: 1px solid green; width: 83%">
				<?php
					$overall = mysqli_query($connect, "SELECT SUM(test + test2) AS total FROM mid_term_results WHERE class ='{$class}' && session = '{$session}'  && term = '{$term}' && student_id = '{$user['student_id']}' && test + exam + test2 > 0");
					...
					$percentage = $r['total']* 100  / (40 * mysqli_num_rows($subject_select));
