...
<html>
<head>
	<title>View User's Result | Fidelity Schools</title>
...
<div class="">
  ...
  while ($clas = mysqli_fetch_array($c_query)) {
   ...
   ?>
    ...
    <td>
     <a href="view_user?id=<?php echo $id; ?>&class=<?php echo $class; ?>&term=<?php echo $term; ?>&session=<?php echo $session; ?>">
...
$u_query = mysqli_query($connect, $u_sql);
	...
	$count_sql = "SELECT DISTINCT student_id FROM mid_term_results WHERE class ='{$class}'   && session = '{$session}'  && term = '{$term}' && test+ test2+exam >0 ORDER BY id DESC";
	$count_query = mysqli_query($connect, $count_sql);
	$counter = mysqli_num_rows($count_query);
...
<div class="col-md-11 m-auto p-3" style=" border: 5px dashed green;">
  ...
  <div class="">
    ...
    <?php if($term == 3){ ?>
				<table  class="w-100 text-center table-responsive-xl"  style="border: 2px solid green;  font-size: 18px; color: #000;">
				<thead>
...
<span>
<a href="view_user?id=<?php echo md5($user['student_id']); ?>">
