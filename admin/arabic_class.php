<?php 
		...
		if (isset($_POST['submit'])) {
			...
			$sel = "SELECT * FROM arabic_class WHERE class = '{$class}'";
			$selt = mysqli_query($connect, $sel);
			$count = mysqli_num_rows($selt);
			if ($count > 0) {
				$msg = "<div class='text-center text-danger'>Class is already existing </div>";
			}
			else{
			$sql = "INSERT INTO arabic_class (class) VALUES ('$class')";
			$query = mysqli_query($connect, $sql);
			if ($query) {
				$msg = "<div class='text-success text-center'>Arabic Class Added Successfully</div>";
			}
			else{
...
<html>
<head>
	<title>Arabic Classes | Fidelity Schools</title>
...
<body>
           ...
           <div class="col-md-10 p-4">
           		<h4 class="ml-3 mt-3 text-center">بسم الله الرحمن الرحيم</h4>
           	<?php 
           		include 'inc/hr.php';
           	 ?>
                  ...
                  <form method="post" enctype="multipart/form-data">
                    <h5 class="font-weight-bold">Add Arabic Classes</h5>
                    <?php echo $msg; ?>
  ...
  <?php 
 $u_sql = "SELECT * FROM arabic_class";
