...
<h3 class=""><?php
	$select = mysqli_query($connect, "SELECT * FROM session WHERE status = 1 ORDER BY id DESC LIMIT 1");
	$c = mysqli_num_rows($select);
	if($c < 1){
		$w['id'] = 1;	
	}else{

		$w = mysqli_fetch_array($select);
	}
	
	$sql = "SELECT * FROM students WHERE class ='{$_SESSION['class']}' && session = '{$w['id']}'";
	$query = mysqli_query($connect, $sql);
...
<h3 class=""><?php  
	$sql = "SELECT * FROM session WHERE status = 1 ORDER BY id DESC LIMIT 1";
	$query = mysqli_query($connect, $sql);
	foreach($query as $cl){
		echo $cl['session'];
	}
...
<h3 class=""><?php  
	$sql = "SELECT * FROM mid_term_results WHERE teacher = '{$_SESSION['id']}'";
	$query = mysqli_query($connect, $sql);
	echo number_format(mysqli_num_rows($query));
	?></h3>
<b>Midterm Reports</b>
...
<h3 class=""><?php  
	$sql = "SELECT * FROM teachers WHERE name = '{$_SESSION['id']}'";
	$query = mysqli_query($connect, $sql);
	echo number_format(mysqli_num_rows($query));
	?></h3>
<b>Subjects</b>
...
<h3 class=""><?php  
	$sql = "SELECT * FROM materials WHERE class = '{$_SESSION['class']}' || class = 'all'";
	$query = mysqli_query($connect, $sql);
	echo number_format(mysqli_num_rows($query));
	?></h3>
<b>materials</b>
...
<tbody>
	<?php  
	$sql = "SELECT * FROM register WHERE level =1  ORDER BY id DESC LIMIT 10";
	$query=  mysqli_query($connect, $sql);
...
<tbody>
	<?php  
	$sql = "SELECT * FROM mid_term_results WHERE teacher = '{$_SESSION['id']}' ORDER BY id DESC LIMIT 10";
	$query=  mysqli_query($connect, $sql);
	while ($tab = mysqli_fetch_array($query)) {
