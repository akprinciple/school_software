...
<html>
<head>
	<title>Manage Results | Fidelity Schools</title>
...
<div class="col-md-10 ">
<h4 class="ml-3 mt-3">All Results</h4>
<?php 
include 'inc/hr.php';
?>
<div class="mt-3 row">
...
<form class="col-md-8 row border-right mx-0">
	...
	?>
	<?php 
	$sql = "SELECT * FROM teachers WHERE name = '{$_SESSION['id']}' ORDER BY subject ASC";
	...
	<?php 
		$a = mysqli_query($connect, "SELECT * FROM subjects WHERE id = '{$row['subject']}'");
		foreach($a as $b){
			echo $b['subject'];
		}
	 ?></option>
...
<tr>
	...
	<td>
		<?php echo $report['test'] + $report['exam'] + $report['test2']; ?>
	</td> 
	<td>
...
?>
	 ...
	 <td>
 	<?php echo $z_row['test2'] + $z_row['test'] ?>
 	...
 	<td>
 	<?php echo $z_row['test'] + $z_row['exam'] + $z_row['test2']; ?>
 	</td>
 	<!-- <td>
